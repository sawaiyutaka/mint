# ============================================================
# Multiple imputation using random forest + logistic regression
# Outcome: total 24-month screen time
#          calculated from AF1-AF6
#
# Binary outcomes:
#   1. screen_ge_1h: childscreen24M >= 1 hour/day
#   2. screen_ge_2h: childscreen24M >= 2 hours/day
#   3. screen_ge_3h: childscreen24M >= 3 hours/day
#
...
# Models:
#   1. Unadjusted:
#      binary outcome ~ education_group1 + education_group2
#
#   2. Adjusted:
#      binary outcome ~ education_group1 + education_group2
#                       + age_corrected
...
#   Education dummy variables and binary screen-time outcomes
#   are created after imputation.
# ============================================================
...
output_file <- "D:/mint/results/logistic_regression_MI_random_forest_mother_education_dummy_childscreen24M_cutoffs.xlsx"
...
# =========================
# 5. Create continuous outcome variable before imputation
# =========================
...
# =========================
# 7. Descriptive statistics before imputation
# =========================
...
cutoff_summary_before <- data.frame(
  cutoff = c(">=1 hour/day", ">=2 hours/day", ">=3 hours/day"),
  n_event = c(
    sum(df[[outcome]] >= 1, na.rm = TRUE),
    sum(df[[outcome]] >= 2, na.rm = TRUE),
    sum(df[[outcome]] >= 3, na.rm = TRUE)
  ),
  n_observed = childscreen24M_n
)

cutoff_summary_before$percent <- round(
  cutoff_summary_before$n_event / cutoff_summary_before$n_observed * 100,
  1
)

print(cutoff_summary_before)

# =========================
# 8. Histogram
# =========================

hist_file <- "D:/mint/results/hist_childscreen24M_overflow8_mother_education_dummy.png"
...
# Keep education as factor for imputation
...
# =========================
# 13. Multiple imputation using random forest
# =========================
...
cutoff_settings <- data.frame(
  outcome_binary = c("screen_ge_1h", "screen_ge_2h", "screen_ge_3h"),
  cutoff_hours = c(1, 2, 3),
  label = c(
...
fit_logistic_for_cutoff <- function(imp, cutoff_hours, outcome_binary_name) {
  ...
  for (i in seq_along(completed_list)) {
    ...
    # Ensure education category has the correct levels
    d[[education]] <- factor(
      d[[education]],
      levels = c("group0", "group1", "group2")
    )
    
    # Create dummy variables after imputation
    d[[education_group1_dummy]] <- ifelse(d[[education]] == "group1", 1, 0)
    d[[education_group2_dummy]] <- ifelse(d[[education]] == "group2", 1, 0)
    
    # Binary outcome after imputation
    d[[outcome_binary_name]] <- ifelse(
      d[[outcome]] >= cutoff_hours,
      1,
      0
    )
    
    d[[outcome_binary_name]] <- as.numeric(d[[outcome_binary_name]])
    
    # Unadjusted model
    fit_unadj_list[[i]] <- glm(
      as.formula(paste0(
        outcome_binary_name, " ~ ",
        education_group1_dummy, " + ",
        education_group2_dummy
      )),
      data = d,
...
# =========================
# 15. Function to format pooled logistic regression results
# =========================

format_logistic_results <- function(
    pooled_object,
    model_label,
    outcome_binary,
    outcome_label,
  ...
  out <- data.frame(
    ...
    ci_lower_log_odds = s[[ci_lower_col]],
    ci_upper_log_odds = s[[ci_upper_col]],
    ci_lower_or = exp(s[[ci_lower_col]]),
    ci_upper_or = exp(s[[ci_upper_col]]),
    p_value = s$p.value,
    n = n_analysis,
    m = m_imp,
...
# =========================
# 16. Run logistic regression for all cutoffs
# =========================

all_results <- data.frame()

for (j in seq_len(nrow(cutoff_settings))) {
  
  outcome_binary_name <- cutoff_settings$outcome_binary[j]
  cutoff_hours        <- cutoff_settings$cutoff_hours[j]
  outcome_label       <- cutoff_settings$label[j]
  
  cat("Running logistic regression for:", outcome_label, "\n")
  
  fits <- fit_logistic_for_cutoff(
    imp = imp,
    cutoff_hours = cutoff_hours,
    outcome_binary_name = outcome_binary_name
  )
  ...
  results_unadj <- format_logistic_results(
    pooled_object = pool_unadj,
    model_label = "unadjusted",
    outcome_binary = outcome_binary_name,
    outcome_label = outcome_label,
    cutoff_hours = cutoff_hours,
  ...
  results_adj <- format_logistic_results(
...
addWorksheet(wb, "screen_ge_1h")
writeData(
  wb,
  "screen_ge_1h",
  all_results %>% filter(outcome == "screen_ge_1h")
)

addWorksheet(wb, "screen_ge_2h")
writeData(
  wb,
  "screen_ge_2h",
  all_results %>% filter(outcome == "screen_ge_2h")
)

addWorksheet(wb, "screen_ge_3h")
writeData(
  wb,
  "screen_ge_3h",
  all_results %>% filter(outcome == "screen_ge_3h")
)

addWorksheet(wb, "education_summary_before")
writeData(wb, "education_summary_before", education_summary_before)

addWorksheet(wb, "cutoff_summary_before")
writeData(wb, "cutoff_summary_before", cutoff_summary_before)
...
imputation_settings <- data.frame(
  ...
  value = c(
    ...
    paste(cutoff_settings$outcome_binary, collapse = ", "),
    "Created after imputation: childscreen24M >= 1, >= 2, and >= 3 hours/day",
    education,
